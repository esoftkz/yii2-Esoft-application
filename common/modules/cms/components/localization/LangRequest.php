...
class LangRequest extends Request
{
    ...
    public function getLangUrl()
    {
        if ($this->_lang_url === null) {
			...
			//Получаем string первой части Url - в нашем случае 'url языка'
        	$url_list = explode('/', $this->_lang_url);
        	$lang_url = isset($url_list[1]) ? $url_list[1] : null;

			//Если $lang_url not null ? $lang_url : default language;
        	Language::setCurrent($lang_url);

			if( $lang_url !== null && $lang_url === Language::getCurrent()->url && strpos($this->_lang_url, Language::getCurrent()->url) === 1 )
			{
	...
	//Просто resolvePathInfo, вообщем ничего особенного.
